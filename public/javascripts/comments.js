(function (window) {
  ...
  function initWidget(widgetEl) {
    ...
    if ((websiteId = widgetEl.getAttribute("data-comments-app-website"))) {
      ...
      widgetElId =
        "comments-app-" +
        websiteId.replace(/[^a-z0-9_]/gi, "-") +
        "-" +
        ++window._CommentsAppWidgetUuid;
      ...
      src =
        ...
        (websitePageId ? "&page_id=" + encodeURIComponent(websitePageId) : "") +
        "&page_url=" +
        encodeURIComponent(websitePageUrl) +
        "&origin=" +
        encodeURIComponent(websiteOrigin) +
        (websitePageTitle
          ? "&page_title=" + encodeURIComponent(websitePageTitle)
          : "") +
        (websiteCommentId
    ...
    function postMessageHandler(event) {
      ...
      if (data.event == "resize") {
        ...
        try {
          window.scrollTo(
            0,
            iframe.getBoundingClientRect().top + window.pageYOffset + data.y
          );
        } catch (e) {}
      }
    }
